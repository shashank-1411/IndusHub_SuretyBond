...
export function CtaButton({ label, href, type = "button" }: CtaButtonProps) {
  const content = (
    ...
    >
      ...
      <span className="absolute inset-y-1 right-1 z-10 grid w-1/4 place-items-center rounded-sm bg-white/10 text-white transition-all duration-500 group-hover:w-[calc(100%-0.5rem)] group-active:scale-95">
        {/* Diagonal arrow icon */}
